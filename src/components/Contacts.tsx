...
function Contacts() {
  ...
  useEffect(() => {
    const handleMessageReceive = (msg: { reciever: string; text: string; createdAt: string }) => {
      console.log(contactList,msg)
      setContactList((prevContacts) =>
        prevContacts.map((contact) =>
          contact._id === msg.reciever
            ? { ...contact, lastMessage: { text: msg.text, createdAt: msg.createdAt } }
            : contact
        )
      );
              ...
              <div className="flex items-center gap-4 py-3 bg-darkGray rounded-lg hover:bg-stone-700">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm text-gray-400">
                    {contact.lastMessage ? contact.lastMessage.text : "Say Hi..."}
                  </span>
                </div>
